...
class SeedPriceService(
    ...
    enum class PriceUnit {
        ...
        companion object {
            fun fromString(string: String): Pair<PriceUnit, Float> {
                return if (string == "PKT") {
                    PKT to 1f
                } else if (string.endsWith("kg", true)) {
                    KG to string.substring(0, string.length - 2).toFloat()
                } else if (string.endsWith("g", true)) {
                    G to string.substring(0, string.length - 1).toFloat()
                } else if (string.endsWith("s", true)) {
                    NUMBER to string.substring(0, string.length - 1).toFloat()
                } else {
                    throw Exception("Invalid units - $string")
                }
            }
        }
    }
    ...
    fun parseCropRow(
        line: String,
        currentHeaderRow: Pair<String, List<Pair<PriceUnit, Float>>>,
    ): SeedData {
        val parts = line.split(",").filter(String::isNotBlank)
        val name = parts[1]
        val costs =
            currentHeaderRow.second.mapIndexedNotNull { index, s ->
    ...
    fun getRowType(line: String): RowType {
        ...
        return if (parts.isEmpty()) {
            RowType.BLANK
        } else if (parts.first() == "PKT") {
            RowType.REUNIT
        } else if (parts.any { it == "PKT" }) {
            RowType.HEADER
        } else {
            RowType.CROP
        }
    ...
    fun getSeedPriceForCropInCountryOnDate(
        ...
        val (cropPrice, priceUnit) =
            getSeedPriceForCrop(crop)?.maxBulk()
                ?: (
                    cropSellingService
    ...
    fun SeedData.maxBulk(): Pair<Pair<PriceUnit, Float>, Float>? {
        ...
        val maxUnit =
            newSeedData.prices.maxBy {
                it.key.first.ordinal
            }.key.first
        val maxValue =
            newSeedData.prices.filter { it.key.first == maxUnit }.maxBy {
                it.key.second
            }
