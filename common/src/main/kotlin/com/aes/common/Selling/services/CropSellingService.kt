...
class CropSellingService(
    ...
    private fun linearJoinPoint(from: CropPrice, to: CropPrice, atDate: LocalDate): Float{
        ...
        val dt = to.middleDate().atStartOfDay().toEpochSecond(ZoneOffset.UTC) - from.middleDate().atStartOfDay().toEpochSecond(ZoneOffset.UTC)

        val dtAtDate = atDate.atStartOfDay().toEpochSecond(ZoneOffset.UTC) - from.middleDate().atStartOfDay().toEpochSecond(ZoneOffset.UTC)
    ...
    private fun weightedExtrapolate(atDate: LocalDate, allData: List<CropPrice>): Float{
        ...
        val extrapolations = (1 until list.size).map {
            linearJoinPoint(list[0], list[it], atDate) to (atDate.atStartOfDay().toEpochSecond(ZoneOffset.UTC) - list[it].middleDate().atStartOfDay().toEpochSecond(ZoneOffset.UTC))
        }
